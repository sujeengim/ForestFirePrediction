...
class Img2ImgModel:
    ...
    @staticmethod
    def random_sublist(lst): #사용안하고 잇음
        sublist = []
    ...
    def generate_images(
        ...
        """
        input_image_path = f"{seed_path}/{seed_image_identifier}.jpg"
        ...
        for i in range(num_images):
            ...
            output_image_path = os.path.join(
                save_path,
                f"{seed_image_identifier}_{'_'.join(variation.split())}_{i}.jpg",
            )
...
if __name__ == "__main__":
    ...
    fire_dir  = 'test-stableDiffusion/jm-test-data/train/fire'
    nofire_dir = 'test-stableDiffusion/jm-test-data/train/no_fire'

    image_urls = {
        "fire": os.listdir(fire_dir),
        "no_fire" : os.listdir(nofire_dir)
    }

    model = Img2ImgModel(model_id, device="cuda") #print model loaded
    num_images = 5
    gen_img_count = 0
    ...
    try:
        ...
        for class_name, urls in image_urls.items():
            for url in urls:
                ...
                input_dir = f"test-stableDiffusion/jm-test-data/train/{class_name}"
                output_dir = f"test-stableDiffusion/jm-test-outdata/{class_name}"
